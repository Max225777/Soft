...
export default function Balance({ me, lang, balanceDiff }: Props) {
  ...
  return (
    <div className="page" style={{ paddingTop: 12 }}>
      ...
      {/* СБП (Platega) card — временно отключено */}
      <div style={{
        background: 'var(--card)',
        border: '1px solid var(--border)',
        borderRadius: 14, marginBottom: 8, overflow: 'hidden', opacity: 0.55,
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', cursor: 'not-allowed' }}>
          <div style={{
            width: 38, height: 38, borderRadius: 11, flexShrink: 0,
            background: 'rgba(255,255,255,.05)', border: '1px solid var(--border)',
            display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, filter: 'grayscale(1)',
          }}>🏦</div>
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--muted)' }}>СБП</div>
            <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 2 }}>
              {lang === 'ru' ? '⏳ Временно недоступно' : lang === 'ua' ? '⏳ Тимчасово недоступно' : '⏳ Temporarily unavailable'}
            </div>
          </div>
          <div style={{
            fontSize: 10, fontWeight: 700, color: 'var(--muted)', border: '1px solid var(--border)',
