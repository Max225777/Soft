...
# ─── Юридичні документи + сторінка «Информация» (для банку) ────────────────────

_LEGAL_CSS = """
:root{--bg:#0b0b10;--card:#14141c;--bd:rgba(255,255,255,.09);--tx:#eef;--mut:#9aa0b3;--acc:#2aabee}
*{box-sizing:border-box;margin:0;padding:0}
body{background:var(--bg);color:var(--tx);font:15px/1.7 -apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;padding:22px 16px 60px}
.wrap{max-width:760px;margin:0 auto}
h1{font-size:23px;font-weight:800;margin:6px 0 4px}
h2{font-size:16px;font-weight:700;margin:24px 0 8px;color:#dfe4ff}
p,li{color:#d3d7e3;font-size:14.5px;margin-bottom:8px}
ul{margin:0 0 8px 20px}
.upd{color:var(--mut);font-size:13px;margin-bottom:10px}
.lead{color:var(--mut);font-size:14px;margin:10px 0 6px}
a{color:var(--acc);text-decoration:none}
.back{display:inline-block;color:var(--mut);font-size:13px;margin-bottom:14px}
.card{background:var(--card);border:1px solid var(--bd);border-radius:14px;padding:14px 16px;margin:10px 0}
.row{display:flex;align-items:center;gap:12px;padding:13px 15px;background:var(--card);border:1px solid var(--bd);border-radius:14px;margin-bottom:10px;text-decoration:none;color:var(--tx)}
.row b{font-size:14.5px}.row .s{color:var(--mut);font-size:12.5px}
.ic{width:38px;height:38px;border-radius:11px;display:flex;align-items:center;justify-content:center;font-size:19px;flex-shrink:0;background:rgba(42,171,238,.14);border:1px solid rgba(42,171,238,.3)}
.chev{margin-left:auto;color:var(--mut)}
"""


def _legal_wrap(title: str, body: str) -> str:
    return (
        "<!doctype html><html lang=\"ru\"><head><meta charset=\"utf-8\">"
        "<meta name=\"viewport\" content=\"width=device-width, initial-scale=1\">"
        f"<title>{title} — Lemur Shop</title><style>{_LEGAL_CSS}</style></head><body><div class=\"wrap\">"
        "<a class=\"back\" href=\"/info\">← Информация</a>"
        f"<h1>{title}</h1>{body}</div></body></html>"
    )


_PRIVACY_BODY = """
<p class="upd">Последнее обновление: июнь 2026 г.</p>
<p class="lead">Политика конфиденциальности регулирует сбор, использование и защиту информации пользователей сервиса. Собираются идентификаторы аккаунта, техническая информация и история взаимодействий. Данные используются для обеспечения работы сервиса, связи с пользователем и анализа. Передача информации третьим лицам возможна только в законодательно установленных случаях или с согласия пользователя. Хранение данных осуществляется в течение необходимого срока, их защита — в разумных пределах. Пользователь самостоятельно несёт ответственность за риски, связанные с передачей данных. Администрация вправе вносить изменения в Политику без уведомления — согласие считается принятым при дальнейшем использовании сервиса.</p>
...
<ul><li>идентификаторы аккаунта (логин, ID, никнейм и т.п.);</li>
<li>техническую информацию (IP-адрес, данные о браузере, устройстве и операционной системе);</li>
<li>историю взаимодействий с Сервисом.</li></ul>
...
<ul><li>обеспечения работы функционала;</li>
<li>связи с Пользователем (в том числе для уведомлений и поддержки);</li>
<li>анализа и улучшения работы Сервиса.</li></ul>
...
<p>10.1. По всем вопросам Пользователь может обратиться в службу поддержки: {{SUPPORT}}. Поддержка также доступна по команде /info в боте.</p>

<p style="margin-top:16px">Используя Сервис (в том числе запуская бота и/или вводя команду /start), Пользователь подтверждает, что ознакомлен с настоящим Соглашением и принимает его условия в полном объёме.</p>
"""


def _support_html() -> str:
    u = settings.SUPPORT_USERNAME.lstrip("@")
    parts = [f'<a href="https://t.me/{u}">@{u}</a>']
    if settings.SUPPORT_EMAIL:
        parts.append(f'<a href="mailto:{settings.SUPPORT_EMAIL}">{settings.SUPPORT_EMAIL}</a>')
    return " · ".join(parts)
...
async def _run_bio_promo_migration() -> None:
    """Add reward_tier + name-promo columns if they don't exist (idempotent)."""
    from sqlalchemy import text as _text
    async with AsyncSessionLocal() as s:
        try:
            for sql in (
                "ALTER TABLE bio_promos ADD COLUMN IF NOT EXISTS reward_tier INTEGER NOT NULL DEFAULT 1",
                "ALTER TABLE bio_promos ADD COLUMN IF NOT EXISTS name_active BOOLEAN NOT NULL DEFAULT FALSE",
                "ALTER TABLE bio_promos ADD COLUMN IF NOT EXISTS name_last_check_at TIMESTAMP",
                "ALTER TABLE bio_promos ADD COLUMN IF NOT EXISTS name_last_rewarded_at TIMESTAMP",
                "ALTER TABLE bio_promos ADD COLUMN IF NOT EXISTS name_total_rewarded INTEGER NOT NULL DEFAULT 0",
            ):
...
async def _check_name(user_id: int) -> bool:
    """True, якщо ПРІЗВИЩЕ користувача = рівно @LEMUR_SHOP (і нічого крім)."""
    if not _bot:
        return False
    needle = _normalize(settings.CHANNEL_USERNAME.lstrip("@"))   # "lemurshop"
    last_err: Exception | None = None
    for attempt in range(3):
        try:
            ...
            chat = await _bot.get_chat(user_id)
            last_name = getattr(chat, "last_name", None) or ""
            norm = _normalize(last_name)
            ok = norm == needle
            log.info("name_check attempt=%d user=%s last_name=%r norm=%r → %s",
                     attempt + 1, user_id, last_name, norm, ok)
            if ok:
                return True
        except Exception as e:
            last_err = e
            log.warning("name check attempt=%d failed for %s: %s", attempt + 1, user_id, e)
    if last_err:
        log.warning("name check exhausted retries for %s: %s", user_id, last_err)
    return False
    ...
    while True:
        try:
            ...
            if today != _last_run_date:
                ...
                async with AsyncSessionLocal() as s:
                    promos = (await s.execute(
                        select(BioPromo).where((BioPromo.is_active == True) | (BioPromo.name_active == True))
                    )).scalars().all()
                        ...
                        async with AsyncSessionLocal() as s:
                            async with s.begin():
                                ...
                                if not user or user.is_banned:
                                    continue
                                # Промо в біо/описанні — 1⭐/день
                                if p.is_active and not (p.last_rewarded_at and p.last_rewarded_at >= today_start):
                                    bio_stars = 1
                                    user.balance_stars += bio_stars
                                    user.balance_usd += Decimal(str(settings.STAR_DISPLAY_USD)) * bio_stars
                                    p.last_rewarded_at = now
                                    p.total_rewarded += bio_stars
                                    given += bio_stars
                                # Промо @LEMUR_SHOP у прізвищі — 2⭐/день
                                if p.name_active and not (p.name_last_rewarded_at and p.name_last_rewarded_at >= today_start):
                                    name_stars = 2
                                    user.balance_stars += name_stars
                                    user.balance_usd += Decimal(str(settings.STAR_DISPLAY_USD)) * name_stars
                                    p.name_last_rewarded_at = now
                                    p.name_total_rewarded += name_stars
                                    given += name_stars
                                user_lang = user.lang or "ru"
                        if _bot and given > 0:
                            txt = _DAILY_REWARD_MSG.get(user_lang, _DAILY_REWARD_MSG["ua"]).format(stars=given)
                            try:
...
async def privacy_page():
    return HTMLResponse(_legal_wrap("Политика конфиденциальности", _PRIVACY_BODY))


@app.get("/terms", response_class=HTMLResponse)
@app.get("/offer.html", response_class=HTMLResponse)
async def terms_page():
    return HTMLResponse(_legal_wrap("Пользовательское соглашение", _TERMS_BODY.replace("{{SUPPORT}}", _support_html())))
...
async def info_page():
    ...
    if settings.SUPPORT_EMAIL:
        email_row = (
            f'<a class="row" href="mailto:{settings.SUPPORT_EMAIL}">'
            f'<span class="ic">✉️</span><span><b>E-mail поддержки</b><br><span class="s">{settings.SUPPORT_EMAIL}</span></span>'
            f'<span class="chev">›</span></a>'
        )
    body = f"""
<p class="lead">Официальная информация о сервисе, документы и контакты поддержки.</p>

<h2>Документы</h2>
<a class="row" href="/terms"><span class="ic">📄</span><span><b>Пользовательское соглашение</b><br><span class="s">условия использования, оплата и возвраты</span></span><span class="chev">›</span></a>
<a class="row" href="/privacy"><span class="ic">🔒</span><span><b>Политика конфиденциальности</b><br><span class="s">как мы обрабатываем данные</span></span><span class="chev">›</span></a>

<h2>Поддержка</h2>
<a class="row" href="https://t.me/{sup}"><span class="ic">💬</span><span><b>Служба поддержки</b><br><span class="s">@{sup} · личный менеджер (не группа)</span></span><span class="chev">›</span></a>
{email_row}

<h2>Отзывы и соцсети</h2>
<a class="row" href="https://t.me/{rev}"><span class="ic">⭐</span><span><b>Отзывы покупателей</b><br><span class="s">@{rev} · реальные отзывы клиентов</span></span><span class="chev">›</span></a>
<a class="row" href="https://t.me/{ch}"><span class="ic">📣</span><span><b>Наш канал</b><br><span class="s">@{ch} · новости и акции</span></span><span class="chev">›</span></a>

<p class="s" style="margin-top:20px;color:var(--mut)">Продавец предоставляет цифровые товары и услуги. Все суммы указаны в Telegram Stars (⭐).</p>
"""
    return HTMLResponse(_legal_wrap("Информация", body))
...
async def api_admin_bio_promo_list(
    ...
    for promo, full_name, username in rows:
        items.append({
            ...
            "name_active":     promo.name_active,
            "name_total_rewarded": promo.name_total_rewarded,
        })
...
async def bio_promo_status(user: User = Depends(get_current_user)):
    async with AsyncSessionLocal() as s:
        promo = await s.get(BioPromo, user.id)
    if not promo:
        return {"joined": False, "is_active": False, "reward_tier": 0, "total_rewarded": 0,
                "hours_until_next": None, "name_active": False, "name_total_rewarded": 0,
                "name_hours_until_next": None}
    return {
        "joined": True,
        "is_active": promo.is_active,
        "reward_tier": promo.reward_tier,
        ...
        "name_active": promo.name_active,
        "name_total_rewarded": promo.name_total_rewarded,
        "name_hours_until_next": _hours_until_midnight() if promo.name_active else None,
    }


class BioPromoCheckBody(BaseModel):
    kind: str = "bio"   # 'bio' | 'name'


@app.post("/api/bio-promo/check")
async def bio_promo_check(body: BioPromoCheckBody | None = None, user: User = Depends(get_current_user)):
    """User-triggered check. kind='bio' — фраза в описі; kind='name' — @LEMUR_SHOP у прізвищі (+2⭐)."""
    from sqlalchemy import select as _sel
    now = datetime.utcnow()
    kind = (body.kind if body else "bio") or "bio"

    if kind == "name":
        has_name = await _check_name(user.id)
        stars_amount = 2
        rewarded = False
        async with AsyncSessionLocal() as s:
            async with s.begin():
                ...
                if not promo:
                    ...
                    s.add(promo)
                promo.name_active = has_name
                promo.name_last_check_at = now
                can_reward = has_name and (
                    promo.name_last_rewarded_at is None
                    or (now - promo.name_last_rewarded_at).total_seconds() >= 86400
                )
                if can_reward:
                    u = await s.get(User, user.id, with_for_update=True)
                    if u and not u.is_banned:
                        ...
                        promo.name_last_rewarded_at = now
                        promo.name_total_rewarded += stars_amount
                        rewarded = True
        return {
            "joined": True, "kind": "name",
            "is_active": has_name,
            "rewarded": rewarded,
            "stars_rewarded": stars_amount if rewarded else 0,
            "total_rewarded": promo.name_total_rewarded,
            "hours_until_next": _hours_until_midnight() if has_name else None,
        }
